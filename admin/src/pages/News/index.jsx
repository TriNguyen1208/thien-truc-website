...
export default function News() {
    ...
    const {data: allNews, isLoading: isLoadingAllResults} = useNews.news.getList(
        "",
        undefined,
        undefined
    )
    ...
    const columnWidths = ["10%", "10%", "28%", "15%", "13%", "13%", "15%"];

    // Định nghĩa tiêu đề cột
    const columns = ["Mã tin tức", "Ảnh", "Tiêu đề", "Trạng thái", "Ngày xuất bản", "Thao tác"];
    const convertNewsListToTableData = (newsList) => {
        return newsList.map((item) => {
            return [
                ...
                { 
                    type: "text", 
                    content: item.is_published ? item.public_date : "Chưa xuất bản"  || "Cập nhật sau"
                },
                {
        ...
        <div>
            <div className="flex flex-wrap gap-5 mt-3 mb-4">
                <div className = "bg-white w-[32%] pl-6 py-3 border border-gray-200 rounded-lg shadow-md justify-left">
                    ...
                    <p className="text-[25px] font-bold text-gray-900 pl-1 mt-1">{allNews.length}</p>
                    <p className="text-[14px] text-gray-500 mb-2">Bài viết</p>
                </div>
                <div className = "bg-white w-[32%] pl-6 py-3 border border-gray-200 rounded-lg shadow-md justify-left">
                    <h1 className="text-xl font-semibold text-gray-1300 mt-2">Đã xuất bản</h1>
                    <p className="text-[25px] font-bold text-green-600 pl-1 mt-1">{allNews?.filter(item => item.is_published === true).length ?? 0}</p>
                    <p className="text-[14px] text-gray-500 mb-2">Bài viết</p>
                </div>
                <div className = "bg-white w-[32%] pl-6 py-3 border border-gray-200 rounded-lg shadow-md justify-left">
                    <h1 className="text-xl font-semibold text-gray-1300 mt-2">Bản nháp</h1>
                    <p className="text-[25px] font-bold text-red-500 pl-1 mt-1">{allNews?.filter(item => item.is_published === false).length ?? 0}</p>
                    <p className="text-[14px] text-gray-500 mb-2">Bài viết</p>
                </div>
            </div>
            ...
            <div>
                {Object.values(newsPage).map(({category, news }) => {
                    const dataTable = convertNewsListToTableData(news);
                    return (
