...
const Product = () => {
    ...
    var categoriesDefault = "Tẩt cả sản phẩm";
    const [searchParams, setSearchParams] = useSearchParams();

    // Lay params
    var id = searchParams.get('id') || undefined;
    var filter = searchParams.get('filter') || undefined;
    ...
    //Lấy id hiện tại mà product edit
    const [idCurrentEditProduct, setIdCurrentEditProduct] = useState(null);
    //Ở thông báo xác nhận
    const [openNotification, setOpenNotification] = useState(false);
    //Lưu trạng thái edit để in ra trong dynamic form
    ...
    //Lưu biến categories: Có 'Tất cả sản phẩm và loại cụ thể lấy từ database
    ...
    //Set lại edit product có thêm options là object gồm có tất cả sản phẩm và các loại lấy từ database
    useEffect(() => {
        const object = [
            ...categories.map(item => ({
                label: item,
    ...
    const convertProductListToTableData = (productList) => {
        return productList.map((product) => {
            return [
                ...
                { 
                    type: "text", 
                    content: `${product.price !== "" ? Number(product.price).toLocaleString('vi-VN') + ' đ' : 'Cập nhật sau'} ` 
                },
                { 
                    type: "text", 
                    content: `${product.warranty_period !== "" ? product.warranty_period + ' tháng' : "Cập nhật sau"}` 
                },
                {
                    type: "component",
                    component: (
