...
export default function Project () {
    //========================= API =======================
    const { mutate: deleteOne, isPending: isPendingDeleting} = useProjects.projects.deleteOne();
    const { mutate: updateFeatureOne, isPending: isPendingUpdate} = useProjects.projects.updateFeatureOne();
    const { data: projectRegions, isLoading: isLoadingProjectRegions } = useProjects.project_regions.getAll();

    //Lấy regions và display
    const regions = [
      'Tất cả khu vực',
    ...
    //Bỏ projects vừa mới fetch vào projects list(chưa được sort)
    ...
    const convertProjectsListToTableData = (projectsList) => {
        return projectsList.map((item) => {
            return [
                ...
                { 
                    type: "text", 
                    content: item.province || 'Cập nhật sau' 
                },
                { 
                    type: "text", 
                    content: item.complete_time ? new Date(item.complete_time).toLocaleDateString('vi-VN') : 'Cập nhật sau' 
                },
                {
                    type: "component",
                    component: (
      ...
      <div>
        ...
        <div>
            {Object.values(projectPage).map(({region, projects}) => {
                const projectsTable = convertProjectsListToTableData(projects);
                return (
